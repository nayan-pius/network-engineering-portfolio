# Network Engineering Portfolio 🚀
**Author:** Pius Nayan  
**Role:** IT Student (Level 200) | Aspiring Network Automation Engineer  
**Status:** Active Learning 🟢

---

## 📖 About This Project
This repository documents my journey from **CCNA fundamentals** to **Network Automation**. Every day, I build a new network topology, troubleshoot real-world scenarios, and push the code here.

## 🛠️ Tech Stack & Tools
* **Simulation:** Cisco Packet Tracer
* **Version Control:** Git & GitHub
* **Documentation:** Markdown & VS Code
* **Future:** Python, Ansible (Coming Soon)

---

## 📅 Daily Progress Log

| Day | Topic | Lab Type | Status |
| :--- | :--- | :--- | :--- |
| **01** | Physical Layer & Cabling | Peer-to-Peer Network | ✅ Completed |
| **02** | Switching & MAC Addresses | Small Office LAN | ✅ Completed |
| **03** | Routers & Gateways | Inter-Network Routing | 🔜 Up Next |
| **04** | VLANs & Segmentation | Traffic Separation | 🔒 Locked |

---

## 📂 Quick Links
* [**View Lab Diagrams**](./diagrams/)
* [**Read Learning Journal**](./journal/)